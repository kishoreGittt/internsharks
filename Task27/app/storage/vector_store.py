...
def _get_user_dir(owner_id: str):
    directory = (
...
def _metadata_file(owner_id: str):
    return (
        _get_user_dir(owner_id)
        / "metadata.json"
    )
...
def load_vectors(owner_id: str):

    file = _index_file(owner_id)
...
def load_metadata(owner_id: str):

    file = _metadata_file(owner_id)
...
def save_vectors(
    ...
    _metadata_file(owner_id).write_text(
        json.dumps(
            metadata,
            indent=2
        ),
